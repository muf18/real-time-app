...
class BitgetAdapter(ExchangeAdapter):
    ...
    async def connect_and_subscribe(self) -> AsyncGenerator[PriceUpdate, None]:
        async with websockets.connect(self.WSS_URL) as websocket:
            subscribe_msg = {
                "op": "subscribe",
                "args": [{"instType": "SPOT", "channel": "trade", "instId": self.exchange_symbol}]
            }
            ...
            while True:
                ...
                if message.get("action") == "snapshot" and message.get("arg", {}).get("channel") == "trade":
    ...
    def _normalize_trade(self, trade: List[str]) -> PriceUpdate:
        ...
        exchange_ts = datetime.fromtimestamp(int(trade[0]) / 1000, tz=timezone.utc).isoformat()
    ...
    async def fetch_historical_data(self, timeframe: str, limit: int) -> List[Candle]:
        # Bitget uses seconds for granularity: e.g. 60, 300, 900
        ...
        params = {"symbol": self.exchange_symbol, "granularity": granularity, "limit": limit}
        ...
        async with httpx.AsyncClient() as client:
            ...
            candles = []
            # Data format: [ts, o, h, l, c, vol, vol]
            for row in data:
                 candles.append(Candle(
                    symbol=self.symbol,
                    timeframe=timeframe,
                    open_time_utc=datetime.fromtimestamp(int(row[0]) / 1000, tz=timezone.utc).isoformat(),
                    open=str(row[1]),
                    high=str(row[2]),
                    low=str(row[3]),
                    close=str(row[4]),
