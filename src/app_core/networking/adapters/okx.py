...
class OKXAdapter(ExchangeAdapter):
    ...
    async def connect_and_subscribe(self) -> AsyncGenerator[PriceUpdate, None]:
        async with websockets.connect(self.WSS_URL) as websocket:
            ...
            while True:
                ...
                # OKX sends heartbeats as 'ping', respond with 'pong'
    ...
    def _normalize_trade(self, trade: Dict[str, Any]) -> PriceUpdate:
        ...
        exchange_ts = datetime.fromtimestamp(int(trade['ts']) / 1000, tz=timezone.utc).isoformat()
    ...
    async def fetch_historical_data(self, timeframe: str, limit: int) -> List[Candle]:
        # OKX bar format e.g. "1m", "5m", "1H", "4H", "1D"
        bar_map = {"1m":"1m", "5m":"5m", "15m":"15m", "30m":"30m", "1h":"1H", "4h":"4H", "1d":"1D", "1w":"1W"}
        ...
        async with httpx.AsyncClient() as client:
            ...
            candles = []
            # Data format: [ts, o, h, l, c, vol, volCcy, ...]
            for row in data:
