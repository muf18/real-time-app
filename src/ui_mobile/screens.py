...
class ChartScreen(Screen):
    # Properties linked to widgets in the .kv file
    ...
    def __init__(self, **kwargs):
        ...
        self._data_buffer = deque(maxlen=100) # Smaller buffer for mobile
    ...
    def load_initial_data(self):
        ...
        # Setup plots if not already done
        if not self.price_plot:
            self.price_plot = LinePlot(color=[0, 1, 0, 1], line_width=2) # Green
            self.vwap_plot = LinePlot(color=[0, 1, 1, 1], line_width=1.5) # Cyan
        ...
        self.controller.load_historical_data(self.current_symbol, self.current_timeframe)
