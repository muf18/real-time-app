...
class UIController(QObject):
    ...
    It manages the async worker thread and uses signals to pass data to the UI thread.
    """
    ...
    def __init__(self):
        ...
        # Connect the worker's run method to the thread's started signal
        self._thread.started.connect(self._async_worker.run)
        
        # Start the thread and the asyncio event loop
        self._thread.start()
    ...
    async def _fetch_historical_data_async(self, symbol: str, timeframe: str):
        ...
        # For simplicity, we'll use the first available adapter for historical data
        exchange_name = config.exchange_integrations[symbol][0]
        adapter_class = self._async_worker.connection_manager.ADAPTER_MAP.get(exchange_name)
        if adapter_class:
    ...
    def shutdown(self):
        ...
        print("UIController: Shutting down...")
        future = asyncio.run_coroutine_threadsafe(
            self._async_worker.stop_all_async(),
            self._async_worker.loop
        )
        future.result() # Wait for the async shutdown to complete
        self._thread.quit()
        self._thread.wait()
        print("UIController: Shutdown complete.")